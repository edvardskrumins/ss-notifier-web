...
import { Link, useRouter } from "@/app/lib/navigation";
    ...
    const fetchData = async () => {
      try {
        ...
        if (!notificationData.category) {
          throw new Error(t('noCategory'));
        }

        const categoryLocale = notificationData.category.locale || 'lv';
        const filtersData = await getAds(notificationData.category.id, categoryLocale);
      ...
      <div className="flex flex-col items-center rounded-3xl border border-zinc-800/80 bg-zinc-900/70 p-8 shadow-xl shadow-black/20">
        <div className="flex w-full flex-col items-center gap-8">
          ...
          <div className="mx-auto w-full space-y-2 border-b border-zinc-800/70 pb-6 text-left sm:w-[85%] md:w-[75%]">
            ...
            <select
              name="sid"
              defaultValue={initialValues.sid || ''}

...
export default function ProfilPage() {
  ...
  const loadUser = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const message = await extractErrorMessage(response, t('unableToLoad'));
        setError(message);
        setUser(null);
        return;
      }
  ...
  const handleResendVerification = useCallback(async () => {
    ...
    try {
      ...
      const message = await extractErrorMessage(
        response,
        response.ok ? t('verificationEmailSent') : t('unableToResendVerification')
      );
      ...
      setVerificationAlert({
        type: "error",
        message: t('unableToResendVerification'),
      });
    } finally {
  ...
  const handlePasswordSubmit = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      if (password !== passwordConfirmation) {
        setPasswordErrors([t('passwordsDoNotMatch')]);
        return;
      }
      ...
      try {
        ...
        if (response.status === 422) {
          ...
          setPasswordAlert({
            ...
            message:
              validationErrors[0] ??
              (await extractErrorMessage(response, t('unableToUpdatePassword'))),
          });
          return;
        }
        ...
        if (!response.ok) {
          setPasswordAlert({
            type: "error",
            message: await extractErrorMessage(response, t('unableToUpdatePassword')),
          });
          return;
        }
        ...
        setPasswordAlert({
          type: "error",
          message: t('unableToUpdatePassword'),
        });
      } finally {
          ...
          <>
            <section className="mt-8 rounded-2xl border border-zinc-800 bg-zinc-950/60 p-6">
              <div className="flex flex-col gap-6 md:flex-row md:justify-between">
                ...
                <div className="md:text-right">
                  <p className="text-sm font-semibold text-zinc-400">{t('email')}</p>
              ...
              <div className="mt-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div className="flex items-center gap-3">
                  <span
                    ...
                    title={isVerified ? t('emailVerified') : t('emailNotVerified')}
                  >
                    {isVerified ? (
                ...
                {!isVerified && (
                  <div className="flex flex-col items-start gap-3 md:flex-row md:items-center md:gap-4">
                    ...
                    >
                      {verificationPending ? t('sending') : t('resendVerification')}
                    </button>
                  </div>
                )}
              </div>
            ...
            <section className="mt-8 rounded-2xl border border-zinc-800 bg-zinc-950/60 p-6">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  <h2 className="text-lg font-semibold text-zinc-100">{t('changePassword')}</h2>
              ...
              >
                <div className="overflow-hidden">
                  ...
                  >
                    ...
                    >
                      {passwordPending ? t('saving') : t('saveNewPassword')}
                    </button>
                  </form>
                </div>
              </div>
